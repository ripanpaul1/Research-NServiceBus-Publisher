...
namespace Lateetud.NServiceBus.ServiceControl.Classes.MsmqReturnToSourceQueue
{
    class ErrorManager
    {
        ...
        public virtual MsmqAddress InputQueue
        {
            set
            {
                ...
                var messageReadPropertyFilter = new MessagePropertyFilter
                {
                    ...
                    Body = true,
                    CorrelationId = true,
                    DefaultBodySize = 10240,
                    DefaultExtensionSize = 2550,
                    DefaultLabelSize = 2550,
                    Id = true,
                    Label = true,
                    MessageType = true,
                    ResponseQueue = true,
        ...
        public List<Models.MessageDetails> ReturnAllMessages()
        {
            
            List<Models.MessageDetails> md = new List<Models.MessageDetails>();
            foreach (var m in queue.GetAllMessages())
            {
                var message = queue.ReceiveById(m.Id, TimeoutDuration, MessageQueueTransactionType.Automatic);
                var headers = MsmqUtilities.ExtractHeaders(message);
                var originalId = GetOriginalId(headers);
                md.Add(new Models.MessageDetails { NServiceBusMessageId = GetOriginalId(headers), QueueMessageId = m.Id });
            }
